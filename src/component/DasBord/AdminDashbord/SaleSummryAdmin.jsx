...
const SaleSummryAdmin = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(`/pagination?page=${page}`);
        ...
        <div className="shadow">
          <div className="">
            <div className="grid grid-cols-3 p-4">
              <div className="">
                <div className="flex flex-col justify-center items-center  border-2 border-t-4 border-t-pink-200 rounded-lg py-4 ">
          ...
          <div className="modal-box">
            ...
            <div>
              <form ref={form} onSubmit={sendEmail}>
                ...
                <input
                  defaultValue={email}
                  className="border-pink-300 px-4 rounded-md border w-full py-4"
                  type="email"
                  name="user_email"
                />
                <label>Message</label>
                <textarea
                  className="border border-pink-300 px-4  rounded-md w-full py-4"
                  name="message"
                />
                <input
                  className="border-pink-300 bg-red-400 text-white text-xl font-bold rounded-md border w-full py-4"
                  type="submit"
                  value="Send"
                />
              </form>
